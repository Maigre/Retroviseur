...
export function develop(src: HTMLCanvasElement, opts: DevelopOptions): HTMLCanvasElement {
	...
	// the stamp sits bottom-right, digits ~2.1 % of the short side tall (D43)
	let stampTex: WebGLTexture | null = null;
	let rect: [number, number, number, number] = [0, 0, 0, 0];
	if (opts.stamp) {
		const stamp = drawStamp(opts.stamp, Math.min(w, h) * 0.021);
		stampTex = L.texture(stamp);
		const margin = Math.min(w, h) * 0.045 - Math.min(w, h) * 0.021 * 0.7;
		const x1 = (w - margin) / w;
